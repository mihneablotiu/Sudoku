...
void solve_sudoku_serial(sudoku_t *sudoku)
{
	int row, col;
...
void solve_sudoku_parallel(sudoku_t *sudoku, int row, int col, int num, int depth)
{
	sudoku->grid[row][col] = num;

	if (depth > sudoku->grid_size) {
		solve_sudoku_serial(sudoku);
		sudoku_free(&sudoku);
	...
	for (num = 1; num <= sudoku->grid_size; num++) {
		if (is_safe(sudoku, row, col, num)) {
			...
			#pragma omp task firstprivate(new_sudoku,row,col,num,depth)
			solve_sudoku_parallel(new_sudoku, row, col, num, depth + 1);
		}
	}
